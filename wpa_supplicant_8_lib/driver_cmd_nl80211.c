...
//<-- Add for VTS test END
    ...
    if ((ret = ioctl(drv->global->ioctl_sock, SIOCDEVPRIVATE + 6, &ifr)) < 0) {
        wpa_printf(MSG_ERROR, "%s: failed to issue private commands(%s), total_len=%d, addr = " MACSTR "\n",
                 __func__, strerror(errno), priv_cmd.total_len, MAC2STR(priv_cmd.buf + 11));
        ret = -1;
    } else {
        wpa_printf(MSG_DEBUG, "%s: Success to set P2PMACADDR " MACSTR, __func__, MAC2STR(priv_cmd.buf + 11));
    }
...
//NOTE: Bug#474462 Add for SoftAp Advance Feature BEG-->
      ...
      if (add) {
          ...
          hostapd->iconf->bss[0]->accept_mac = newacl;
          os_memcpy(newacl[hostapd->iconf->bss[0]->num_accept_mac].addr, addr, ETH_ALEN);
          hostapd->iconf->bss[0]->num_accept_mac++;
      }
      ret = os_snprintf(buf, buf_len, "OK");
      ...
      i = 0;
      while (i < hostapd->iconf->bss[0]->num_accept_mac) {
          if (os_memcmp(acl[i].addr, addr, ETH_ALEN) == 0) {
              os_remove_in_array(acl, hostapd->iconf->bss[0]->num_accept_mac, sizeof(*acl), i);
              hostapd->iconf->bss[0]->num_accept_mac--;
              break;
          }
          i++;
      ...
      ret = os_snprintf(buf, buf_len, "OK");
        ...
        if (android_priv_cmd(priv, cmd_buf) < 0)
            return -1;
        hostapd->iconf->bss[0]->macaddr_acl = DENY_UNLESS_ACCEPTED;
        ret = os_snprintf(buf, buf_len, "OK");
        ...
        if (android_priv_cmd(priv, cmd_buf) < 0)
            return -1;
        hostapd->iconf->bss[0]->macaddr_acl = ACCEPT_UNLESS_DENIED;
        ret = os_snprintf(buf, buf_len, "OK");
